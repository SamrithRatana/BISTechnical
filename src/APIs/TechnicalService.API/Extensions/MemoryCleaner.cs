...
/// <summary>
/// High-performance cross-platform memory cleaner.
...
/// <summary>
/// Autonomous background service that performs scheduled and adaptive memory maintenance.
...
public sealed class MemoryMaintenanceHostedService : BackgroundService
{
    ...
    {
        _logger.LogInformation("Automatic Memory Maintenance Service initialized (Default Interval: {Interval}m).", DefaultInterval.TotalMinutes);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (savedMb > 5.0)
                {
                    _logger.LogInformation(
                        "[MemoryMaintenance] Automatic memory purge completed. WorkingSet: {BeforeMb}MB -> {AfterMb}MB (Saved: {SavedMb}MB, GC Heap: {GcHeapMb}MB).",
            ...
            {
                _logger.LogWarning(ex, "[MemoryMaintenance] Error occurred during memory cleanup cycle.");
            }
