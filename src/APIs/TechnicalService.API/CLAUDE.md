...
- **Classification on the list, search, by-id, create and update (2026-09-05).** `SparepartSearchQuery` takes `categoryId` / `typeId` / `brandId` (Guid.Empty = no filter; all three are in the output-cache vary key) and `sortBy=brand`; `searchTerm` also matches the brand / type / category **names** (LEFT JOINs on the optional navigations, one statement). The `Sparepart` view model carries `CategoryId/CategoryName/TypeId/TypeName/BrandId/BrandName/BrandLogoUrl`, all nullable — additive, so the phone app's older reader is unaffected. `POST`/`PUT /spareparts` accept an optional `classification: { categoryId, typeId, brandId }` object (`Application/Commands/SparepartClassification.cs`, validated against the taxonomy: category exists, type ∈ category, brand exists → 400). **Absent object = leave the part's classification alone** (the CAM ID app still sends the pre-taxonomy payload); present object replaces all three, nulls/empty guids clear. Both endpoints evict the `spareparts` **and** `sparepart-taxonomy` tags (the lookups carry `PartCount`).
- `DELETE /spareparts/{sparepartId:guid}` → `SparepartTaxonomyApi.DeleteSparepartAsync` → `DeleteSparepartCommand` (added 2026-09-05 — both the web and the CAM ID app had been calling it against nothing). Refuses with **409** while the part has ticket lines or any row in the stock audit ledger (`FK_AuditLog_Spareparts` is NO ACTION), so in practice only a never-moved part can go; there is no FK from `SparepartItems` to `Spareparts` (33 orphans already exist), so the ticket-line check has no DB backstop.

Spare-part taxonomy (`src/APIs/TechnicalService.API/Apis/SparepartTaxonomyApi.cs`, mapped from the same versioned builder in `Program.cs` so `RequireAuthorization()` covers it; schema in `sql/sparepart-taxonomy.sql`, **not** an EF migration — the snapshot is behind the live DB):
- `GET /spareparts/categories` · `POST` · `PUT /{id:guid}` · `DELETE /{id:guid}` → `Create/Update/DeleteSparepartCategoryCommand`
- `GET /spareparts/types?categoryId=` · `POST` · `PUT /{id:guid}` · `DELETE /{id:guid}` → `Create/Update/DeleteSparepartTypeCommand` (a type belongs to one category; re-parenting a type that parts use is refused)
- `GET /spareparts/brands` · `POST` · `PUT /{id:guid}` · `DELETE /{id:guid}` → `Create/Update/DeleteSparepartBrandCommand` (name stored UPPER-CASE — domain normalisation + `CK_SparepartBrands_Name_Upper`; `logoUrl` optional, absolute http(s) only)
- Lists come from `ISparepartTaxonomyQueries` (`SparepartTaxonomyQueries.cs`) with `TypeCount`/`PartCount` as correlated sub-selects in one statement. GETs are output-cached under policy/tag `sparepart-taxonomy` (5 min, `VaryByQuery categoryId`); every write evicts that tag **and** `spareparts`, with `CancellationToken.None` because the write has already committed.
- Errors are exceptions: `KeyNotFoundException` → 404, `RequestValidationException` / `TechnicalServiceDomainException` → 400 (`ValidationExceptionHandler`), `ConflictException` → 409 with ProblemDetails `code` (`duplicate` | `inUse`) and `count` (`ConflictExceptionHandler`, which also maps a raced `DbUpdateException` — SQL 2601/2627 → `duplicate`, 547 → `constraint`, logged at Warning).
- `Create/UpdateSparepartAsync` and `ManualStockOutAsync` now evict the `spareparts` tag; until 2026-09-05 nothing did, so a new or edited part was invisible for up to 60s. `POST/PUT /inspectitem` and `DELETE /inspectitem/.../spareparts/...` also move stock through triggers and still do **not** evict — open item.
