...
export default function AiAssistantPanel() {
  ...
  return createPortal(
    <aside
      ...
      className="fixed top-0 right-0 z-[200] h-screen w-[min(23rem,100vw)] xl:w-[min(26rem,100vw)] flex flex-col bg-surface border-l border-subtle shadow-2xl dropdown-panel-in"
    >
      {/* ── Top Header Bar (Clean Enterprise Surface) ────────────────── */}
      <div className="sticky top-0 z-20 flex items-center justify-between px-4 py-3.5 border-b border-subtle bg-surface text-ink shadow-xs">
        <div className="flex items-center gap-3 min-w-0">
          <div className="relative shrink-0">
            <div className="w-9 h-9 rounded-xl bg-accent-soft border border-accent/25 flex items-center justify-center text-accent shadow-xs">
              <Bot className="w-5 h-5" />
            </div>
            <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-500 ring-2 ring-surface shadow-xs" />
          </div>
          ...
          <div className="flex flex-col min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="inline-flex items-center gap-1 text-[10px] font-medium text-ink-muted">
                ...
                {isKhmer ? "ប្រព័ន្ធឆ្លើយតបផ្ទាល់" : "Live Assistant"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 text-ink-secondary shrink-0">
          <button
            ...
            title={t("ai.model")}
            className="relative p-1.5 rounded-xl hover:text-ink hover:bg-cushion transition-colors cursor-pointer"
          >
            <Cpu className="w-4 h-4" />
            {limitedCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 min-w-[1rem] h-3.5 px-1 rounded-full bg-warning text-[9px] font-bold text-warning-fg flex items-center justify-center">
                {limitedCount}
              </span>
            )}
          </button>
          ...
          {ai.messages.length > 0 && (
            <button
              ...
              className="p-1.5 rounded-xl hover:text-ink hover:bg-cushion transition-colors cursor-pointer"
            >
      ...
      {/* ── Transcript ─────────────────────────────────────────────────── */}
      <div className="flex-1 overflow-y-auto px-3.5 py-4 space-y-4 bg-cushion/60">
        {ai.messages.length === 0 && !ai.loading && (
          <div className="flex flex-col items-center justify-center h-full text-center px-4 py-8">
            <div className="w-12 h-12 rounded-2xl bg-accent-soft text-accent border border-accent/20 flex items-center justify-center mb-3 shadow-xs">
        ...
        {ai.loading && (
          <div className="flex items-start gap-2.5">
            <div className="w-7 h-7 rounded-xl bg-accent-soft text-accent border border-accent/20 flex items-center justify-center shrink-0 text-xs shadow-xs">
              <Bot className="w-4 h-4" />
            </div>
            <div className="px-3.5 py-2.5 rounded-2xl rounded-tl-none bg-surface border border-subtle text-xs text-ink-secondary flex items-center gap-2 shadow-xs">
      ...
      <div className="border-t border-subtle bg-surface p-3.5">
        <div className="flex items-center gap-2 bg-cushion border border-subtle/80 rounded-full px-4 py-1.5 focus-within:ring-2 focus-within:ring-accent/25 focus-within:border-accent focus-within:bg-surface transition-all shadow-xs">
          <input
            type="text"
            value={draft}
          ...
          <button
            ...
            title={t("ai.send")}
            className="shrink-0 w-8 h-8 rounded-full bg-accent text-white flex items-center justify-center hover:opacity-90 disabled:opacity-40 transition-all shadow-xs cursor-pointer active:scale-95"
          >
            {ai.loading ? (
...
function Bubble({ message, quotaWait }: { message: ChatMessage; quotaWait: number | null }) {
  ...
  if (message.role === "user") {
    return (
      <div className="flex flex-col items-end gap-1">
        <div className="max-w-[85%] px-4 py-2.5 rounded-2xl rounded-br-none bg-accent text-white text-[13px] leading-relaxed shadow-xs whitespace-pre-line font-medium">
          {message.text}
        </div>
  ...
  return (
    <div className="flex items-start gap-2.5 max-w-[95%]">
      <div className="w-7 h-7 rounded-xl bg-accent-soft text-accent border border-accent/20 flex items-center justify-center shrink-0 text-xs mt-0.5 shadow-xs">
        <Bot className="w-4 h-4" />
      </div>

      <div className="flex flex-col items-start gap-1 min-w-0 flex-1">
        <div className="w-full px-4 py-3 rounded-2xl rounded-tl-none bg-surface border border-subtle shadow-xs">
          {message.text ? (
            ...
            <figure className="mt-2 first:mt-0">
              ...
              <figcaption className="flex items-center justify-between gap-2 mt-1.5 text-[10px] text-ink-muted">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1 shrink-0 px-1.5 py-0.5 rounded hover:text-accent hover:bg-cushion transition-colors"
                >
          ...
          {/* Image failures are explained in the reply text itself */}
          ...
          {hasBadges && (
            <div className="flex flex-wrap items-center gap-1.5 mt-2.5 pt-2 border-t border-subtle text-[10px]">
        ...
        <div className="flex items-center justify-between w-full px-1 text-[10px] text-ink-muted">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              title="Copy"
              className="p-1 rounded-lg hover:text-ink hover:bg-cushion transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              title="Helpful"
              className={`p-1 rounded-lg transition-colors cursor-pointer ${liked === true ? "text-accent font-bold bg-accent-soft" : "hover:text-ink hover:bg-cushion"}`}
            >
            ...
            <button
              ...
              title="Not helpful"
              className={`p-1 rounded-lg transition-colors cursor-pointer ${liked === false ? "text-danger font-bold bg-danger-soft" : "hover:text-ink hover:bg-cushion"}`}
            >
...
function ModelOption({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex items-center gap-2 px-2.5 py-1.5 rounded-xl text-[11px] text-left transition-colors cursor-pointer ${
        selected
          ? "bg-accent-soft text-accent-soft-fg border border-accent/20 font-semibold"
          : "text-ink-secondary hover:bg-cushion"
      }`}
    >
      <span
      ...
      {note && (
        <span className={selected ? "text-accent" : "text-warning"}>
          {note}
        </span>
      )}
