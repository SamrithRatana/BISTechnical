...
export default function LifecycleRail({ stages, mode, onOpenTopic }: LifecycleRailProps) {
  ...
  return (
    <section ref={railRef} id="lifecycle" className="scroll-mt-24 border-y border-white/5 py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        >
          <p className="mb-2 font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500">
          ...
          <p className={`mt-3 text-sm leading-relaxed text-slate-400 ${isKhmer ? "leading-loose" : ""}`}>
        ...
        <div className="-mx-4 overflow-x-auto px-4 pb-3 sm:mx-0 sm:px-0">
          ...
          <div className="relative w-max min-w-full">
            ...
            <ol className="relative flex items-stretch gap-3">
              {stages.map((stage, i) => {
                const skin = ACCENT[stage.accent];
                return (
                  <motion.li
                    key={stage.status}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 pl-1">
                      <motion.span
                        className={`flex h-[22px] w-[22px] items-center justify-center rounded-full border font-mono text-[10px] font-bold ${skin.chip} bg-[#05060f]`}
                    ...
                    <button
                      ...
                      className={`mt-3 w-full flex-1 cursor-pointer rounded-2xl border bg-[#080a14] p-3 text-left transition-transform duration-150 hover:-translate-y-0.5 ${skin.border}`}
                    >
                      <span className="block font-mono text-[10px] font-bold leading-tight text-slate-500">
                        {stage.status}
                      </span>
        ...
        <p className="mt-4 text-[11px] text-slate-600">{text(LIFECYCLE_COPY.footnote)}</p>
      </div>
    </section>
  );
}
