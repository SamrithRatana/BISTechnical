...
/**
 * @file sales-conversion-report/page.tsx
 * @description Sales Quote Conversion Rate — measures quote approval rates (Sale Confirmed vs Customer Rejected)
 * and average closing days.
 * Columns defined in public/templates/sales-conversion-report.xlsx.
 */
...
import { fetchSalesConversionReport } from "@/services/reports";
...
export default function SalesConversionReportPage() {
  ...
  const load = useCallback(
    async (from: Date, to: Date, filters: ReportFilterValues): Promise<ReportData> => {
      const rows = await fetchSalesConversionReport({
        fromDate: from,
        toDate: to,
  ...
  const format = useCallback(
    (field: string, value: unknown) => {
      if (field === "decisionDate") return formatDayTime(value);
  ...
  return (
    <PageWrapper titleKey="nav.salesConversion" subtitleKey="report.salesConversionTitle">
      <TemplateReportView
        template="sales-conversion-report"
        title={t("report.salesConversionTitle")}
        subtitle={subtitle}
        load={load}
        filters={["search", "serviceType", "location"]}
        format={format}
        fileName="quote-conversion-report"
