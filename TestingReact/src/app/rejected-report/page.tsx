...
/**
 * @file rejected-report/page.tsx
 * @description Rejected & Scrap Report — tracks tickets marked as Customer Rejected or Unrepairable.
 * Columns defined in public/templates/rejected-report.xlsx.
 */
...
import { fetchRejectedReport } from "@/services/reports";
import { formatDay, formatDayTime, groupBy } from "@/services/reportShaping";

export default function RejectedReportPage() {
  const { t } = useI18n();
  ...
  const load = useCallback(
    async (from: Date, to: Date, filters: ReportFilterValues): Promise<ReportData> => {
      const rows = await fetchRejectedReport({
        fromDate: from,
        toDate: to,
  ...
  return (
    <PageWrapper titleKey="nav.rejectedReport" subtitleKey="report.rejectedTitle">
      <TemplateReportView
        template="rejected-report"
        title={t("report.rejectedTitle")}
        subtitle={subtitle}
        load={load}
        filters={["search", "status", "serviceType", "location"]}
        format={format}
        fileName="rejected-scrap-report"
